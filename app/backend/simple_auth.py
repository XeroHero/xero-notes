...
async def get_simple_user_by_username(username: str):
    user = await db.simple_users.find_one({"username": username}, {"_id": 0})
    return user

async def create_simple_user(username: str, password: str, email: Optional[str] = None):
...
async def simple_signup(user_data: SimpleUserCreate):
    ...
    existing_user = await get_simple_user_by_username(user_data.username)
    if existing_user:
        raise HTTPException(status_code=400, detail="Username already exists")
    
    # Create new user
    user_id = await create_simple_user(user_data.username, user_data.password, user_data.email)
...
async def simple_login(login_data: SimpleUserLogin):
    # Find user
    user = await get_simple_user_by_username(login_data.username)
    if not user:
...
async def simple_me(username: str):
    user = await get_simple_user_by_username(username)
    if not user:
